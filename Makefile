...
isort_check: ## check that isort has been run
	isort --check-only -rc edx_rbac_demo/

isort: ## run isort to sort imports in all Python files
	isort --recursive --atomic edx_rbac_demo/

style: ## run Python style checker
	pylint --rcfile=pylintrc edx_rbac_demo *.py
...
pii_check: ## check for PII annotations on all Django models
	DJANGO_SETTINGS_MODULE=edx_rbac_demo.settings.test \
...
dummy_translations: ## generate dummy translation (.po) files
	cd edx_rbac_demo && i18n_tool dummy
...
open-devstack: ## open a shell on the server started by start-devstack
	docker exec -it edx_rbac_demo /edx/app/edx_rbac_demo/devstack.sh open

pkg-devstack: ## build the edx_rbac_demo image from the latest configuration and code
	docker build -t edx_rbac_demo:latest -f docker/build/edx_rbac_demo/Dockerfile git://github.com/edx/configuration

detect_changed_source_translations: ## check if translation files are up-to-date
	cd edx_rbac_demo && i18n_tool changed

validate_translations: fake_translations detect_changed_source_translations ## install fake translations and check if translation files are up-to-date

docker_build:
	docker build . -f Dockerfile -t openedx/edx_rbac_demo
	docker build . -f Dockerfile --target newrelic -t openedx/edx_rbac_demo:latest-newrelic

travis_docker_tag: docker_build
	docker tag openedx/edx_rbac_demo openedx/edx_rbac_demo:$$TRAVIS_COMMIT
	docker tag openedx/edx_rbac_demo:latest-newrelic openedx/edx_rbac_demo:$$TRAVIS_COMMIT-newrelic

travis_docker_auth:
	echo "$$DOCKER_PASSWORD" | docker login -u "$$DOCKER_USERNAME" --password-stdin

travis_docker_push: travis_docker_tag travis_docker_auth ## push to docker hub
	docker push 'openedx/edx_rbac_demo:latest'
...
app-shell: # Run a shell, as root, on the specified service container
	docker exec -u 0 -it edx_rbac_demo.app bash

db-shell:
	docker exec -u 0 -it edx_rbac_demo.db mysql -u root edx_rbac_demo
